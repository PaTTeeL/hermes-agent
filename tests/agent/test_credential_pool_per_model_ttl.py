...
class TestPerModelRateLimitTTL:
    ...
    def test_expired_rate_limit_recovery(self, mock_time, pool_with_entry):
        """After TTL expires, the entry becomes available again but keeps its rate_limited record; only a 200 prunes it."""
        pool, entry = pool_with_entry
        ...
        # Should auto-recover (available again) without pruning the record — only
        # a 200 (clear_rate_limit) prunes it.
        available, _ = pool._available_entries(model_id="gpt-4", clear_expired=True)
        assert len(available) == 1
        assert available[0].id == entry.id
        assert "gpt-4" in available[0].rate_limited  # consecutive kept; only 200 clears
